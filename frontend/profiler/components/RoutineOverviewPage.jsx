...
export const RoutineListHeaderComponent = props => (
  <React.Fragment>
    <h2>Routines</h2>
    <Button size="sm" onClick={() => props.onChangeSorting('name')}>
      Name
    </Button>
    <Button size="sm" onClick={() => props.onChangeSorting('file')}>
      Filename
    </Button>
    <Button size="sm" onClick={() => props.onChangeSorting('entries')}>
      Entries
    </Button>
    <Button size="sm" onClick={() => props.onChangeSorting('inclusive_time')}>
      Inclusive Time
    </Button>
    <Button size="sm" onClick={() => props.onChangeSorting('exclusive_time')}>
      Exclusive Time
    </Button>
  </React.Fragment>
);
...
export default function RoutineOverviewPage(props) {
  ...
  useEffect(() => {
    if (
      ...
      (props.profilerState.routineOverview.length === 0 ||
        props.profilerState.routines.length === 0)
    ) {
      setIsLoading(true);
  ...
  return (
    <ErrorBoundary>
      ...
      {props.profilerState.routineOverview.length === 0 ||
      props.profilerState.routines.length === 0 ? (
        <Button onClick={props.onRequestRoutineOverview}>
